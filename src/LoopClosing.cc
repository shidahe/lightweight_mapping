...
void LoopClosing::SetLocalMesher(LocalMeshing* pLocalMesher)
{
    mpLocalMesher = pLocalMesher;
}
...
//            printf("3 %lf\n", ((double)cvGetTickCount() - t) / (cvGetTickFrequency() * 1000));
            mpLocalMesher->InsertKeyFrame(mpCurrentKF);
        }
...
//        char fileName[1024] ;
//        std::sprintf(fileName,"//home//ygling2008//orb_slam_output//%d.png", mpCurrentKF->mnId ) ;
//        cv::imwrite(fileName, mpCurrentKF->img ) ;
//        //cv::imshow("current KF", mpCurrentKF->img ) ;
...
//            std::sprintf(fileName,"//home//ygling2008//orb_slam_output//%d-%d.png", mpCurrentKF->mnId, mvpEnoughConsistentCandidates[i]->mnId ) ;
//            cv::imwrite(fileName, mvpEnoughConsistentCandidates[i]->img ) ;
...
void LoopClosing::RunGlobalBundleAdjustment(unsigned long nLoopKF)
{
    ...
    double BATime = ((double)cvGetTickCount() - t) / (cvGetTickFrequency() * 1000) ;
    printf("Global BA Time = %lf\n",  BATime ) ;
